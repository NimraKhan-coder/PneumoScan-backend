...
def _next_date_for_day(day_of_week: str) -> date:
    """Returns the next upcoming calendar date matching this day of week
    (today counts as 'next' if it hasn't passed yet -- kept simple here
    and just always picks the next occurrence, including today)."""
    today = date.today()
...
def _generate_slots(start_time: str, end_time: str, duration_minutes: int) -> list[str]:
    """Generates discrete slot start-times between start and end, stepped
    by duration_minutes. e.g. 09:00-17:00 in 30-min steps -> 09:00, 09:30, ..."""
    slots = []
...
def get_available_slots(
    ...
    all_slots = _generate_slots(day_range["start_time"], day_range["end_time"], duration)
    scheduled_date = _next_date_for_day(day_of_week)

    # Exclude slots already booked (pending payment or confirmed) for this
    # doctor on this exact date.
    booked_response = (
    ...
    booked_times = {row["scheduled_time"][:5] for row in booked_response.data}  # "HH:MM"
...
class CreateBookingRequest(BaseModel):
    doctor_id: str
    consultation_type: str  # 'video' or 'voice'
    day_of_week: str
    slot_time: str  # "10:00"
...
def create_booking(payload: CreateBookingRequest, user_id: str = Depends(get_current_user_id)):
    ...
    # Re-check the slot is still free (avoids a race condition between two
    # patients booking the same slot at the same moment).
    existing = (
        supabase.table("consultation_bookings")
        .select("id")
...
def create_payment_intent(booking_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        intent = stripe.PaymentIntent.create(
            amount=int(booking["fee"] * 100),  # smallest currency unit
            currency="pkr",
