...
def get_patient_report_detail(report_id: str, user_id: str = Depends(get_current_user_id)):
    report_response = (
        supabase.table("patient_reports")
        .select("*")
        .eq("id", report_id)
        .execute()
    )
    ...
    if not relationship_check.data:
        raise HTTPException(status_code=403, detail="You do not have access to this report.")

    ai_status = report.get("status", "pending")
    ai_ready = ai_status == "completed" and report.get("ai_analysis") is not None
    ...
    return {
        ...
        "status": ai_status,
        "ai_analysis": report.get("ai_analysis") if ai_ready else None,
        "ai_ready": ai_ready,
        "message": None if ai_ready else "AI analysis not available for this report yet.",
    }