...
@router.patch("/doctor/appointments/{booking_id}/decline")
def decline_booking(booking_id: str, user_id: str = Depends(get_current_user_id)):
    response = (
        supabase.table("consultation_bookings")
        .select("status")
        .eq("id", booking_id)
    ...
    if response.data[0]["status"] != "confirmed":
        raise HTTPException(status_code=400, detail="Only confirmed bookings can be declined.")

    supabase.table("consultation_bookings").update({"status": "declined"}).eq("id", booking_id).execute()
