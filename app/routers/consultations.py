...
def get_appointment_detail(booking_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    latest_prediction = prediction_response.data[0] if prediction_response.data else None

    appointment = _format_appointment(booking, "patient")
    appointment["latest_xray_report"] = latest_prediction  # null if patient never uploaded one
